...
import rasterTableStyles from "../../styles/RasterWmsTable.module.css";
...
class Raster extends Component {
  ...
  fetchRastersFromApi(page, searchContains, include3diScenarios, organisation) {
    ...
    listRastersForTable(
      this.state.pageSize, page, searchContains, include3diScenarios, organisation.uuid
    ).then(({results, count}) => {
              ...
              return (
                <div className={`${rasterTableStyles.tableBody}`} key={raster.uuid}>
                  ...
                  <div className={`${rasterTableStyles.tableUpload}`}>
                    ...
                    {(!raster.is_geoblock && raster.raster_sources.length && !raster.spatial_bounds) ? (
                      <NavLink
                    ...
                    ) : (!raster.is_geoblock && raster.raster_sources.length) ? (
                      <NavLink
                        to={`/data_management/rasters/${raster.uuid}/data`}
                      >
                        <i
