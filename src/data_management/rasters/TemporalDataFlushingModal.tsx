...
function TemporalDataFlushingModal(props: MyProps & DispatchProps) {
  ...
  const [start, setStart] = useState<Date | undefined>(
    firstValueTimestamp ? firstValueTimestamp.toDate() : undefined
  );
  const [stop, setStop] = useState<Date | undefined>(
    lastValueTimestamp ? lastValueTimestamp.toDate() : undefined
  );
  ...
  const startValidator = () => {
    ...
    if (start && lastValueTimestamp && start > lastValueTimestamp.toDate())
      return "Start date must be before the last timestamp.";
    return false;
  };
  const stopValidator = () => {
    if (stop && !moment(stop).isValid()) return "Wrong date format (DD-MM-YYYY hh:mm)";
    if (stop && firstValueTimestamp && stop < firstValueTimestamp.toDate())
      return "End date must be after the first timestamp.";
    return false;
  };
  ...
  return (
    ...
    >
      <p style={{ marginBottom: 10 }}>
        Please select a time range to flush data from <b>{row.name}</b> raster source.
      </p>
      <em>
        Note: Selected time range must be in between the first timestamp and the last timestamp of
        the source.
      </em>
      <div className={styles.GridContainer}>
        <div>
          <span>
            <b>First timestamp of the source:</b>
          </span>
          <br />
        ...
        <div>
          <span>
            <b>Last timestamp of the source:</b>
          </span>
          <br />
      ...
      <div className={styles.GridContainer}>
        <div>
          <b>Start of time range:</b>
        </div>
        <div>
          <b>End of time range:</b>
        </div>
        <Datetime
          initialValue={start}
          onChange={(event) => {
          ...
          isValidDate={(currentDate) =>
            currentDate >= moment(row.first_value_timestamp).subtract(1, "d") &&
            currentDate <= moment(row.last_value_timestamp)
          }
        />
        <Datetime
          initialValue={stop}
      ...
      {busyDeleting ? (
        ...
        >
          <MDSpinner size={96} />
          <span style={{ marginLeft: "20px", fontSize: "19px", fontWeight: "bold" }}>
            Deleting ...
          </span>
        </div>
      ) : null}
