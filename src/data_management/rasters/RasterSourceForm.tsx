...
export const fetchSuppliers = async (uuid: string, searchInput: string) => {
  ...
  return responseJSON.results.map((supplier: User) =>
    convertToSelectObject(supplier.id, supplier.username)
  );
};

const RasterSourceForm: React.FC<Props & DispatchProps & RouteComponentProps<RouteParams>> = (
  props
) => {
  ...
  const initialValues = currentRecord
    ? {
        ...
        interval: currentRecord.interval
          ? toISOValue(rasterIntervalStringServerToDurationObject(currentRecord.interval))
          : "",
    ...
    if (!currentRecord) {
      ...
      createRasterSource(rasterSource)
        ...
        .then((parsedBody) => {
          ...
          sendDataToLizardRecursive(parsedBody.uuid, values.data, values.temporal);
        })
      ...
      patchRasterSource(currentRecord.uuid as string, body)
        .then((data) => {
          ...
          if (status === 200) {
            ...
            props.addFilesToQueue(uploadFiles);
            props.openCloseUploadQueueModal(true);
            sendDataToLizardRecursive(props.match.params.uuid, values.data, values.temporal);
  ...
  return (
    <ExplainSideColumn
      ...
      explanationText={
        rasterSourceFormHelpText[fieldOnFocus] || rasterSourceFormHelpText["default"]
      }
    ...
    >
      <form className={formStyles.Form} onSubmit={handleSubmit} onReset={handleReset}>
        ...
        <div className={formStyles.ButtonContainer}>
          <CancelButton url={"/management/data_management/rasters/sources"} />
          <div style={{ display: "flex" }}>
            {currentRecord ? (
              <div style={{ marginRight: 16 }}>
                <FormActionButtons
                  actions={
                    currentRecord.temporal
                      ? [
                          {
                            displayValue: "Delete",
      ...
      {rasterCreatedModal ? (
        <Modal
          ...
          onClickButtonConfirm={() =>
            props.history.push("/management/data_management/rasters/layers/new")
          }
        >
          <p>A layer is needed to view the raster in the portal.</p>
          <p>
            We automatically created a layer for you to compose. You will now be redirected to the
            layer management.
          </p>
        </Modal>
      ) : null}
      ...
      currentRecord.labeltypes.length === 0 ? (
        <DeleteModal
          rows={[currentRecord]}
          displayContent={[
            { name: "name", width: 65 },
      ...
      {showDeleteModal &&
      currentRecord &&
      (currentRecord.layers.length !== 0 || currentRecord.labeltypes.length !== 0) ? (
