...
const DeleteRasterSourceNotAllowed: React.FC<MyProps> = (props) => {
  const { handleClose, rowToBeDeleted } = props;

  const layerUuids = rowToBeDeleted.layers.map((layerUrl: string) => {
    return layerUrl.split("/")[layerUrl.split("/").length - 2];
  });
  const layerUrls = layerUuids.map((layerUuid: string) => {
    return "/management/data_management/rasters/layers/" + layerUuid;
  });
  const labelTypeUrls = rowToBeDeleted.labeltypes.map((uuid: string) => {
    return "/management/data_management/labeltypes/" + uuid;
  });

  return (
    <Modal title={"Not allowed"} cancelAction={handleClose}>
      <p>
        {"You are trying to delete the raster source "}
        <a
          target="_blank"
          rel="noopener noreferrer"
          href={`/management/data_management/rasters/sources/${rowToBeDeleted.uuid}`}
        >
          {rowToBeDeleted.name}
        </a>
        <br />
