...
export const RasterSource = (props: Node<RasterSourceInput>) => {
  ...
  const [rasterSource, setRasterSource] = useState<Value | null>(
    value ? convertToSelectObject(value) : null
  );
  useEffect(() => {
    if (value) {
      fetchRasterSourceV4(value).then(
        (rasterSource) =>
          rasterSource &&
          rasterSource.uuid &&
          setRasterSource(convertToSelectObject(rasterSource.uuid, rasterSource.name))
      );
    }
  }, [value]);

  return (
    <div className={`${styles.Block} ${styles.RasterSource}`} tabIndex={1}>
      <div className={styles.BlockHeader}>
        <BlockName label={label} onConfirm={onBlockNameChange} />
        <small>
          <i>({classOfBlock})</i>
        </small>
      </div>
      ...
      <div style={{ marginTop: 10 }}>
        <small>{rasterSource?.label}</small>
        <br />
