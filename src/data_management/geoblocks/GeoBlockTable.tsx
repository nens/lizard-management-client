...
export const GeoBlockTable: React.FC<RouteComponentProps> = (props) => {
  ...
  const columnDefinitions: ColumnDefinition<RasterLayerFromAPI>[] = [
    ...
    },
    {
      ...
      renderFunction: (row) => (
        <span className={tableStyles.CellEllipsis} title={row.weight.toString()}>
          {row.weight}
        </span>
      ),
      orderingField: null,
  ...
  return (
    ...
    >
      <TableStateContainer
        ...
        defaultUrlParams={"&scenario__isnull=true&is_geoblock=true"} // to only show geoblocks & to exclude scenario result layers
      />
