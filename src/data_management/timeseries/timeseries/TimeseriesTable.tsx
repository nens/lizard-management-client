...
export const TimeseriesTable = (props: RouteComponentProps) => {
  ...
  const [selectedRowsToAddMN, setSelectedRowsToAddMN] = useState<
  ...
  const columnDefinitions: ColumnDefinition<TimeseriesFromTimeseriesEndpoint>[] = [
    {
      ...
      renderFunction: (row) => (
        <span className={tableStyles.CellEllipsis} title={`${row.name} - UUID: ${row.uuid}`}>
      ...
      renderFunction: (row) => (
        <span
          ...
          title={
            !row.observation_type
              ? undefined
              : row.observation_type.unit
              ? `${row.observation_type.code} (${row.observation_type.unit})`
        ...
        >
          {!row.observation_type ? (
            ...
            <>
              {row.observation_type.code}{" "}
              <span style={{ fontWeight: 600 }}>{`(${row.observation_type.unit})`}</span>
            </>
          ) : (
            row.observation_type.code
          )}
