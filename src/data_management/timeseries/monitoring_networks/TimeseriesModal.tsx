...
function TimeseriesModal(props: MyProps & DispatchProps) {
  ...
  const [currentUrl, setCurrentUrl] = useState<string | null>(
    baseUrl + `?page_size=${itemsPerPage}`
  );
      ...
      .then((parsedResponse) => {
        ...
        setTimeseriesApiResponse({
          previous: parsedResponse.previous
            ? getRelativePathFromUrl(parsedResponse.previous)
            : null,
        ...
        .then((res) => {
          if (res.status === 204) {
            props.addNotification(
              `${timeseriesToDelete.length} time series removed successfully from monitoring network`,
              2000
            );
            props.handleClose();
          } else {
        ...
        <div className={styles.GridContainer}>
          <div className={styles.TimeseriesContainer}>
            <div>
              ...
              >
                <ul className={styles.TimeseriesList}>
                  {timeseriesApiResponse.results.map((ts) => (
                    <li className={styles.TimeseriesRow} key={ts.uuid}>
                      <span
                        style={{
                          textDecoration: timeseriesToDelete.includes(ts.uuid)
                            ? "line-through"
                            : undefined,
                          marginRight: 10,
                        }}
                      ...
                      <button
                        ...
                        onClick={() => {
                          if (timeseriesToDelete.includes(ts.uuid)) {
                            setTimeseriesToDelete(
                              timeseriesToDelete.filter((tsUuid) => tsUuid !== ts.uuid)
                            );
                          } else {
              ...
              >
                <em>
                  <b>{timeseriesToDelete.length} time series selected for deletion</b>
                </em>
              </span>
              <div className={tableStyles.TableSpinner}>
                ...
                dataRetrievalState === "RETRIEVING" ? (
                  ...
                  timeseriesApiResponse.results.length === 0 ? (
          ...
          <div>
            ...
            <p>
              To add time series to a monitoring network, please visit the time series management
              page. There you can search for the time series that you want to add.
            </p>
            <p>
              When you are done adding new time series, please refresh this page to review your
              changes.
            </p>
            <button
