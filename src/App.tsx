...
import {
  getCurrentUrlPostfix,
  getCurrentNavigationLinkPage,
  userHasCorrectRolesForCurrentNavigationLinkTile,
} from "./home/AppTileConfig";
...
const App = (props: RouteComponentProps & DispatchProps) => {
  const { fetchLizardBootstrap, fetchOrganisations, updateTaskStatus, addNotification } = props;
  ...
  useEffect(() => {
    if (
      ...
      !userAuthenticated &&
      !isNotFinishedFetchingBootstrap
    ) {
      setShowUnauthenticatedRedirectModal(true);
    }
  }, [currentNavigationLinkPage, userAuthenticated, isNotFinishedFetchingBootstrap]);

  const userHasCorrectRoles = userHasCorrectRolesForCurrentNavigationLinkTile(
    selectedOrganisation ? selectedOrganisation.roles : []
  );
  useEffect(() => {
    if (
      userAuthenticated &&
  ...
  return (
    <div
      ...
      style={{
        backgroundColor:
          getCurrentUrlPostfix() === "" || getCurrentUrlPostfix() === "/" ? "#F8F8F8" : "white",
      }}
    >
            ...
            >
              <div className={styles.TopNav}>
                <div style={{ display: "none" }}>
                  <a href="#apps">
                    <i className={`material-icons ${styles.AppIcon}`}>apps</i>
                    Apps
                  </a>
                </div>
                {currentRelativeUrl !== "/" ? (
                ...
                {userAuthenticated && selectedOrganisation && !isLandingPage ? (
                  ...
                  >
                    <div>
                      ...
                      {filesInProcess && filesInProcess.length > 0 ? (
                        <span className={styles.NavNotification}>!</span>
                      ) : null}
                      Upload queue
                    </div>
                  </div>
      ...
      {currentRelativeUrl !== "/" ? (
        <div className={`${styles.Secondary}`}>
          <div className={gridStyles.Container}>
            <div className={gridStyles.Row}>
              <Breadcrumbs currentRelativeUrl={currentRelativeUrl} />
            </div>
          </div>
        </div>
      ) : null}
        ...
        <div className={gridStyles.Container}>
          <div className={gridStyles.Row}>
            ...
            >
              ...
              >
                <FormattedMessage id="index.documentation" defaultMessage="Documentation" />
                &nbsp;
                <i className={`${styles.DocumentationHyperlink} material-icons`}>local_library</i>
              </a>
            </div>
            <div
              ...
              <div className={styles.FooterRightWrapper}>
                ...
                <div>
                  <a
                    ...
                    rel="noopener noreferrer"
                  >
                    <i className={`${styles.SupportHyperlink} material-icons`}>headset_mic</i>
                    &nbsp;
                    <FormattedMessage id="index.support" defaultMessage="Support" />
                  </a>
                </div>
              </div>
            </div>
      ...
      {showOrganisationSwitcher ? (
        <OrganisationSwitcher handleClose={() => setShowOrganisationSwitcher(false)} />
      ) : null}
      <Snackbar />
      {showUploadQueue ? (
        <UploadQueue handleClose={() => props.openCloseUploadQueueModal(false)} />
      ) : null}
