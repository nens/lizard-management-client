...
const Snackbar: React.FC<PropsFromDispatch> = (props) => {
  ...
  return (
    <div className={styles.SnackbarWrapper}>
      <div className={`${styles.Snackbar} ${notifications.length > 0 ? styles.Show : styles.Hide}`}>
        <span className={styles.SnackbarMessage}>
          {generateMessage(notifications[notifications.length - 1]) || ""}
        </span>
        <div className={styles.DismissButton} onClick={() => dismiss(notifications.length - 1)}>
          {0 ? (
            <FormattedMessage id="components_snackbar.dismiss" defaultMessage="Dismiss" />
          ) : null}
          Dismiss
        </div>
      </div>
