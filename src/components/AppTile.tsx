...
const AppTile = (props: Props & RouteComponentProps) => {
  const { title, icon, readonly, linkPath, openInNewTab, requiredRoles } = props;
  ...
  const requiresRoleMessageForTranlation = formattedMessageToString(
    <FormattedMessage
  ...
  const requiresRoleMessage =
    requiredRolesLength === 1 ? "Required role: " : "Requires one of the following roles: ";
