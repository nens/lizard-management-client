...
function MapViewer() {
  ...
  const [selectedRasterForReOrdering, setSelectedRasterForReOrdering] = useState<null | string>(
    null
  );

  const moveSelectedRasterUp = () => {
    const ind = selectedRasters.findIndex((item) => {
      return item.uuid === selectedRasterForReOrdering;
    });
          ...
          <div>
            {reversedRasters.map((raster) => {
              return (
                ...
                >
                  <span style={{ width: "280px", /*display: "inline-block",*/ textAlign: "right" }}>
                    {raster.name}{" "}
                  </span>
                  <input
          ...
          >
            {reversedRasters.length > 1 ? (
              <>
                <button
                  type="button"
                  onClick={moveSelectedRasterUp}
                  disabled={!selectedRasterForReOrdering}
                >
                  {" "}
                  ^
                </button>
                <button
                  type="button"
                  onClick={moveSelectedRasterDown}
                  disabled={!selectedRasterForReOrdering}
                >
                  v
                </button>
          ...
          <MapViewerRasterLayerTable
            setSelectedRasters={(rasters) => {
              const uniqueRasters = rasters.filter((raster) => {
                if (
                  selectedRasters.find((selectedRaster) => {
