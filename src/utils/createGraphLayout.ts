...
import { ConnectionLineType, Elements, Position } from "react-flow-renderer";
...
export const createGraphLayout = (
  ...
  elements.forEach((el) => {
    ...
    if (el.data && el.data.parameters) {
      const blockParameters = el.data.parameters.filter(
        (parameter) => typeof parameter === "string" && allBlockNames.includes(parameter)
      );
      blockParameters.forEach((parameter) => {
        dagreGraph.setEdge(parameter + "", el.id);
      });
    }
  });
  ...
  const edges: Elements = elements
    .filter((el) => el.data && el.data.parameters)
    .map((el) => {
