...
export const UploadData: React.FC<MyProps> = (props) => {
  const { title, name, temporal, fileTypes, data, setData, onFocus } = props;
  ...
  const onDrop = (files: File[]) => {
    ...
    const filesData = newFilesNonDuplicates.map((file) => {
      ...
      const dateStrFromUTCNonISO8601WithoutDash = dateStrFromUTCNonISO8601
        ? dateStrFromUTCNonISO8601[0].replaceAll("-", "")
        : null;

      // Use moment.js to re-format the date string from YYYYMMDDTHHMM to YYYY-MM-DDTHH:MM
      const dateStrReformatted = dateStrFromFile
        ? dateStrFromFile[0]
        : dateStrFromUTCFile
        ? moment(dateStrFromUTCFile[0]).format("YYYY-MM-DDTHH:mm:ss")
        : dateStrFromUTCNonISO8601 && dateStrFromUTCNonISO8601WithoutDash
        ? moment(dateStrFromUTCNonISO8601WithoutDash).format("YYYY-MM-DDTHH:mm:ss")
        : null;

      // Add timezone information or Z (UTC time) to the end of the date string
      const dateStrReformattedInUTC =
        dateStrReformatted && dateStrReformatted + (timezoneFromFile ? timezoneFromFile[0] : "Z");

      // Convert to Date object
      const dateObjFromFile: Date | undefined = dateStrReformattedInUTC
        ? new Date(dateStrReformattedInUTC)
        : undefined;
  ...
  const dropZone = (
    <div id={name} className={uploadStyles.DropZoneContainer} {...getRootProps()} onFocus={onFocus}>
      <input {...getInputProps()} />
      {isDragActive ? (
        <span>Drop selected files here ...</span>
      ) : (
        <span>
          Drag and drop {fileTypes.includes(".csv") ? ".csv" : ".tif"} files here or
          <span className={buttonStyles.NewButton} style={{ marginLeft: 10 }}>
            BROWSE
          </span>
        </span>
      )}
  ...
  const fileList = (
    <div className={uploadStyles.FileListContainer}>
      {temporal ? (
        <div>
          {data.map((e, i) => (
            <div key={e.file.name + e.file.size} className={uploadStyles.File}>
              <span className={uploadStyles.FileName}>{e.file.name}</span>
              <span className={uploadStyles.DateTimeContainer}>
                <Datetime
                  value={e.dateTime}
                  onChange={(event) => {
                    let dataCopy = data;
              ...
              <span
                ...
                onClick={() => {
                  const dataCopy = data;
                  const newData = dataCopy.filter((_, iLoc) => i !== iLoc);
                  setData(newData);
                }}
              >
