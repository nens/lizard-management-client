...
export const toApiColorMapStep = (colormapStep: ColormapStep) => {
  return [
    ...
    [
      ...
      colormapStep.rgba.g,
      colormapStep.rgba.b,
      colormapStep.rgba.a === undefined ? 255 : Math.round(colormapStep.rgba.a * 255),
    ],
  ];
};
...
export const ColormapAllSteps: React.FC<Props> = (props) => {
  const { title, steps, name, onChange, colormapIsLogarithmic, colormapIsDiscrete } = props;
    ...
    <div style={{ height: "100%", display: "flex", flexDirection: "column" }}>
      ...
      <div className={styles.StepContainer}>
        {steps.map((step: ColormapStep, ind) => {
          ...
          const errorMsg =
            colormapIsLogarithmic === true && ind === 0 && (step.step === 0 || step.step < 0)
              ? "Logarithmic step must be larger then 0"
              : ind > 0 && step.step < previousStepValue
              ? "Each step must be greater or equal to previous step"
              : colormapIsDiscrete &&
                (step.step + "").match(/^[+-]?\d+(\.\d+)?$/) &&
                (step.step + "").includes(".")
              ? "Discrete steps must be whole numbers"
              : undefined;
              ...
              <div style={{ position: "relative" }}>
                ...
                */}
                <div
                  ...
                  onClick={(event) => {
                    if (visiblepickerIndex === ind && event.currentTarget === event.target) {
                      setVisiblepickerIndex(null);
                    } else {
                      setVisiblepickerIndex(ind);
                    }
                  }}
