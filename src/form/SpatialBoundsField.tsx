...
export const spatialBoundsValidator = (fieldValue: SpatialBoundsProps["value"]) => {
  ...
  const { north, east, south, west } = fieldValue;
...
const SpatialBoundsField = (props: SpatialBoundsProps) => {
  ...
  const { wmsLayerSlug, wmsLayerUrl } = otherValues;
  ...
  useEffect(() => {
    if (
      ...
      if (value.north < value.south) {
        southInput.current.setCustomValidity(
          "South coordinate must be smaller than North coordinate"
        );
      } else {
        southInput.current.setCustomValidity("");
      }
  ...
  return (
    <label htmlFor={name} className={formStyles.Label}>
      ...
      <div className={formStyles.FormGroup + " " + inputStyles.PositionRelative}>
        <div className={styles.InputFields + " " + styles.TextAlignCenter}>
          <label
