...
const PersonalApiKeyFormModel: React.FC<
  Props & PropsFromDispatch & RouteComponentProps<RouteParams>
> = (props) => {
  const { currentRecord, allPersonalApiKeys } = props;
  const recordWithFtpExists =
    (allPersonalApiKeys || []).filter((record) => (record.scope + "").includes("ftp:readwrite"))
      .length > 0;
  ...
  const initialValues = {
    ...
    scopeWildcardReadWrite:
      (currentRecord &&
        currentRecord.scope &&
        (currentRecord.scope + "").includes("*:readwrite")) ||
      true,
    scopeFtpReadWrite:
      (currentRecord &&
        currentRecord.scope &&
        (currentRecord.scope + "").includes("ftp:readwrite")) ||
      false,
  };
  ...
  return (
    <ExplainSideColumn
      ...
      explanationText={
        personalApiKeysFormHelpText[fieldOnFocus] || personalApiKeysFormHelpText["default"]
      }
    ...
    >
      <form className={formStyles.Form} onSubmit={handleSubmit} onReset={handleReset}>
        ...
        />
        <span className={formStyles.FormFieldTitle}>2. Scope</span>
        <CheckBox
          title={"Read / Write"}
          ...
          errorMessage={
            values.scopeWildcardReadWrite === true || values.scopeFtpReadWrite === true
              ? false
              : "Chose at least 1 scope: read/ write or FTP"
          }
        />
        <CheckBox
          title={"FTP"}
          htmlTitle={
            recordWithFtpExists && !currentRecord
              ? "There exists already a record with scope FTP in your api keys. Only one is allowed."
              : "FTP"
          }
        ...
        <div className={formStyles.ButtonContainer}>
          <CancelButton url={"/management/personal_api_keys"} />
