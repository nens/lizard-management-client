...
export const ContactTable: React.FC<RouteComponentProps> = (props) => {
  ...
  const columnDefinitions: ColumnDefinition<Contact>[] = [
    {
      ...
      renderFunction: (row) => (
        ...
        >
          <NavLink to={`${navigationUrl}/${row.id}`}>
            {getDjangoUserOrContactUser(row).first_name}
          </NavLink>
        </span>
      ),
    ...
    },
    {
      ...
      renderFunction: (row) => (
        ...
        >
          <NavLink to={`${navigationUrl}/${row.id}`}>
            {getDjangoUserOrContactUser(row).last_name}
          </NavLink>
        </span>
      ),
    ...
    },
    {
      ...
      renderFunction: (row) => (
        <span className={tableStyles.CellEllipsis} title={getDjangoUserOrContactUser(row).email}>
  ...
  return (
    ...
    >
      ...
      {selectedRow ? (
        <AddToGroupModal contact={selectedRow} handleClose={() => setSelectedRow(null)} />
      ) : null}
    </ExplainSideColumn>
  );
};
