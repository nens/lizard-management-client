...
function AddToGroupModal(props: MyProps & DispatchProps) {
  ...
  const { data: groups, isFetching: groupsIsFetching } = useRecursiveFetch(
  ...
  return (
    ...
    >
      ...
      >
        <div>
          <p>
            Which group(s) would you like to add{" "}
            <b>
          ...
          <p>
            <i>
              Note: only groups that the contact has not yet been added to are shown in the list
            </i>
          </p>
          <SelectDropdown
            title={"Groups"}
        ...
        <div className={formStyles.ButtonContainer}>
          ...
          <SubmitButton onClick={handleSubmit} readOnly={!selectedGroups.length} />
        </div>
      </div>
    </ModalBackground>
  );
