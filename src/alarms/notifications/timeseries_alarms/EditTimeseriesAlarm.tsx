...
export const EditTimeseriesAlarm: React.FC<RouteComponentProps<RouteParams>> = (props) => {
  ...
  const [timeseries, setTimeseries] = useState<TimeseriesFromTimeseriesEndpoint | undefined>(
    undefined
  );
  ...
  useEffect(() => {
    (async () => {
      if (currentRecord) {
        ...
        const timeseries = await createFetchRecordFunctionFromUrl(
          `/api/v4/timeseries/${timeseriesUuid}/`
        )();
