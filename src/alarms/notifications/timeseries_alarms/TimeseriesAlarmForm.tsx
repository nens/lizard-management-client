...
import {
  getTimeseriesLabel,
  TimeseriesFromTimeseriesEndpoint,
  ...
  const initialValues =
    currentRecord && timeseries
      ? {
          ...
          timeseries: convertToSelectObject(
            timeseries.uuid,
            getTimeseriesLabel(timeseries),
            timeseries.uuid,
            "",
            timeseries.location ? timeseries.location.name || timeseries.location.code : ""
          ),
          relative: !!currentRecord.relative_start || !!currentRecord.relative_end,
          relativeStart: currentRecord.relative_start
            ? convertDurationObjToSeconds(
                rasterIntervalStringServerToDurationObject(currentRecord.relative_start)
              )
            : null,
          relativeEnd: currentRecord.relative_end
            ? convertDurationObjToSeconds(
                rasterIntervalStringServerToDurationObject(currentRecord.relative_end)
              )
            : null,
          ...
          recipients: currentRecord.messages.map((message) => {
            ...
            return {
              contact_group: selectedGroup
                ? convertToSelectObject(groupId, selectedGroup.name)
                : convertToSelectObject(groupId),
              message: selectedTemplate
                ? convertToSelectObject(templateId, selectedTemplate.name)
                : convertToSelectObject(templateId),
            };
          }),
        }
      : {
      ...
      <form className={formStyles.Form} onSubmit={handleSubmit} onReset={handleReset}>
        ...
        <AlarmThresholds
          ...
          valueChanged={(threshold) =>
            handleValueChange("thresholds", [...values.thresholds, threshold])
          }
        ...
        <label className={formStyles.Label}>
          <span className={formStyles.LabelTitle}>Snoozing *</span>
          <div className={formStyles.GridContainer}>
            <IntegerInput
        ...
        <Recipients
          ...
          availableTemplates={templates.map((template) =>
            convertToSelectObject(template.id, template.name)
          )}
