...
function InvitationModal(props: MyProps & DispatchProps) {
  ...
  const deleteAction = (row: UserInvitation, triggerReloadWithCurrentPage: Function) => {
    if (!row) return;
  ...
  const columnDefinitions: ColumnDefinition<UserInvitation>[] = [
    ...
    },
    {
      ...
      renderFunction: (row) => {
        ...
        const numberOfDaysToBeExpired =
          numberOfDaysToBeCleanedUp - numberOfDaysCreated >= 0
            ? numberOfDaysToBeCleanedUp - numberOfDaysCreated
            : 0;
        return (
