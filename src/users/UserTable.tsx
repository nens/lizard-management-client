...
export const UserTable: React.FC<RouteComponentProps> = (props) => {
  ...
  const { data: invitationList, isFetching: invitationListIsFetching } = useRecursiveFetch(
    "/api/v4/invitations/",
  ...
  const columnDefinitions: ColumnDefinition<User>[] = [
    ...
    },
    {
      ...
      renderFunction: (row) => (
        <span className={tableStyles.CellEllipsis} title={row.email}>
          {row.email}
        </span>
      ),
      orderingField: "email",
  ...
  return (
    ...
    >
      <TableStateContainer
        ...
        customTableButton={{
          name: invitationListIsFetching
            ? "Loading"
            : invitationList
            ? `${invitationList.length} Pending ${invitationList.length > 1 ? "Users" : "User"}`
            : "0 Pending User",
      ...
      {selectedRows.length > 0 ? (
        <DeleteModal
          ...
          text={
            "You are deactivating the following user. Please make sure that s/he is not a member of any other organisation before continue."
          }
        />
      ) : null}
